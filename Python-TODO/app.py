...
def create():
    if request.method == 'POST':
        ...
        # Create Student object with document content
        ...
        # Add Student object to the database session and commit
...
@app.route('/download_document/<int:student_id>')
def download_document(student_id):
    student = Student.query.get_or_404(student_id)
    if student.document:
        # Create a BytesIO object from the document content
        document_stream = BytesIO(student.document)
        # Send the document as a file download
        # Assuming the document is a PDF for demonstration purposes
    response = Response(document_stream.read(), mimetype='application/pdf')
    response.headers['Content-Disposition'] = f'attachment; filename="{student.name}_{student.surname}_document.pdf"'
    return response
    # else:
